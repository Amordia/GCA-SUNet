...
DATA_PATH = '/home/unnc/CountGD/FSC147_384_V2/'
...
class resizePreTrainImage(object):
    ...
    def __call__(self, sample):
        ...
        W, H = image.size
        # Resize so that divisible by 16
        new_H = 16 * int(H / 16)
        new_W = 16 * int(W / 16)
        '''scale_factor = float(256)/ H
        new_H = 16*int(H*scale_factor/16)
        new_W = 16*int(W*scale_factor/16)'''
        resized_image = transforms.Resize((new_H, new_W), antialias=True)(image)
        resized_density = cv2.resize(density, (new_W, new_H))
        # Select density again
...
class resizeTrainImage(object):
    ...
    def __init__(self, data_path, MAX_HW=384):
    ...
    def __call__(self, sample):
        # Get density map, annotations, etc.
        ...
        # Random self mosaic
            ...
            if dots.shape[0] >= 70:
                for i in range(4):
                    ...
                    reresized_image1 = transforms.Resize((resize_l, resize_l), antialias=True)(reresized_image1)
                ...
                for i in range(4):
                    if i == gt_pos:
                        ...
                        r_image = TTensor(transforms.Resize((new_TH, new_TW), antialias=True)(Timage))
                    ...
                    r_image1 = transforms.Resize((resize_l, resize_l), antialias=True)(r_image1)
        ...
        # Save current density map as gt_map # TODO: Check if this is correct
        gt_map = reresized_density
        # print(gt_map.numpy())
