...
class SumConv2d(nn.Module):
    def __init__(self):
        ...
        kernel_size = 16
        stride = 16
        self.conv = nn.Conv2d(in_channels=1, out_channels=1, kernel_size=kernel_size, stride=stride, padding=0,
                              bias=False)
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler):
    ...
    if loss_scaler is not None:
        checkpoint_paths = [output_dir / f'{train}-checkpoint-{epoch_name}.pth']
...
def load_model(args, model_without_ddp, optimizer, loss_scaler):
    if args.resume:
        ...
        if 'decoder_pos_embed' in checkpoint['model'] and checkpoint['model']['decoder_pos_embed'].shape != \
...
def load_model_FSC(args, model_without_ddp):
    if args.resume:
        ...
        #     print(f"Removing key decoder_pos_embed from pretrained checkpoint")
        #     del checkpoint['model']['decoder_pos_embed']
        if 'decode_head0.0.weight' in checkpoint['model'] and checkpoint['model']['decode_head0.0.weight'].shape != \
                model_without_ddp.state_dict()['decode_head0.0.weight'].shape:
            print(f"Removing key decoder_pos_embed from pretrained checkpoint")
